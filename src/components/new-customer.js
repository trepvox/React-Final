        ...
        <form onSubmit={handleSubmit}>
            <Container id="customer-edit">
                <div className='form-group'>
                    ...
                    <div className="form-group">
                        <label htmlFor="petBreedInput"><h5>Pet's Breed</h5><p>In the event where the breed is the pet type, like a ferret, just repeat the pet type</p></label>
                    ...
                    <Dropdown>
                        ...
                        <Dropdown.Menu>
                            {appointmentDays.map((appointmentDay, i) => <Dropdown.Item eventKey = {i} onSelect={(eventKey) => setAppointmentDay(appointmentDay)}> {appointmentDay} </Dropdown.Item>)}
                        </Dropdown.Menu>
                    </Dropdown>
                        {/* <br></br> */}
                    <Dropdown>
                        ...
                        <Dropdown.Menu>
                            {appointmentTimes.map((appointmentTime, i) => <Dropdown.Item eventKey = {i} onSelect={(eventKey) => setAppointmentTime(appointmentTime)}> {appointmentTime} </Dropdown.Item>)}
                        </Dropdown.Menu>
                    </Dropdown>
                        {/* <br></br> */}
                    <Dropdown>
                        ...
                        <Dropdown.Menu>
                            {walkDurations.map((walkDuration, i) => <Dropdown.Item eventKey = {i} onSelect={(eventKey) => setWalkDuration(walkDuration)}> {walkDuration} </Dropdown.Item>)}
                        </Dropdown.Menu>
                    </Dropdown>
                        <br></br>
